...
public class UserRepository {
    ...
    getData(Constants.SERVER_URL + "/users?login=" + URLEncoder.encode(login, StandardCharsets.UTF_8) +
    ...
    public User delete(long id) throws IOException{
        try (InputStream inputStream = getData(Constants.SERVER_URL + "/users" + "?id=" + id, "DELETE")) {
            ResponseResult<User> userResponseResult = objectMapper.readValue(inputStream,
                    new TypeReference<>() {
                    });
            return userResponseResult.getData();
