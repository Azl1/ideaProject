...
public class CategoryRepository {
    ...
    public Category add(long id, Category category) throws IOException {
        try (InputStream inputStream = getData(Constants.SERVER_URL + "/categories?userId=" + id, "POST", category)) {
    ...
    public Category update(long idUser, Category category) throws IOException {
        try (InputStream inputStream = getData(Constants.SERVER_URL + "/categories?userId=" + idUser, "PUT", category)) {
            ResponseResult<Category> categoryResponseResult = objectMapper.readValue(inputStream,
                    new TypeReference<>() {
                    });
            return categoryResponseResult.getData();
    ...
    public ArrayList<Category> get(long id) throws IOException {
        try (InputStream inputStream = getData(Constants.SERVER_URL + "/categories" + "?userId=" + id, "GET")) {
    ...
    public Category delete(long id) throws IOException{
        try (InputStream inputStream = getData(Constants.SERVER_URL + "/categories?userId=" + id, "DELETE")) {
            ResponseResult<Category> categoryResponseResult = objectMapper.readValue(inputStream,
                    new TypeReference<>() {
                    });
            return categoryResponseResult.getData();
