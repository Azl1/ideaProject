...
public class AutoRepository {
    ...
    public List<Auto> getAll() throws IOException {
        Response<ResponseResult<List<Auto>>> execute = service.getAll().execute();
        return getData(execute);
    }

    public Auto get(long id) throws IOException {
        Response<ResponseResult<Auto>> execute = service.get(id).execute();
        return getData(execute);
    }

    public Auto delete(long id) throws IOException {
        Response<ResponseResult<Auto>> execute = service.delete(id).execute();
        return getData(execute);
    }

    public List<Auto> delete(String name) throws IOException {
        Response<ResponseResult<List<Auto>>> execute = service.delete(name).execute();
        return getData(execute);
    }
