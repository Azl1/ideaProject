package com.abdullaevaziz.fencingschoolspringsecurityfx;

import com.abdullaevaziz.fencingschoolspringsecurityfx.controllers.ControllerData;
    ...
    public void start(Stage stage) throws IOException {
        ...
        if (login != null && password != null && id != -1) {
            try {
                ...
                String token = userRepository1.authenticate(login, password);
                //String token = userRepository1.getTokenUserName(tokenPref);
                String secretKey = "jwtappsecret";

                int idUser = (Integer) Jwts.parser().setSigningKey(Base64.getEncoder().encodeToString(secretKey.getBytes())).parseClaimsJws(token).getBody().get("id");
        ...
        try {
            ...
            stage.setTitle("Project Fencing School!");
