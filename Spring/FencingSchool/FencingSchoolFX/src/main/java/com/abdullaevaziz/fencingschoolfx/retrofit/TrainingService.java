...
public interface TrainingService {
    ...
    @POST("training/{id}/{id}")
    Call<ResponseResult<Training>> post(@Path("id") long trainerId, @Path("id") long apprenticeId, @Body Training training);

    @GET("training/{id}")
    Call<ResponseResult<Training>> getTrainingById(@Path("id") long id);

    @GET("training/trainer/{id}")
    Call<ResponseResult<List<Training>>> getTrainingByTrainerId(@Path("id") long id);
