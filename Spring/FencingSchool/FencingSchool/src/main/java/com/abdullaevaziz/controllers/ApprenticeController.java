...
public class ApprenticeController {
     ...
     */
    @PostMapping
    public ResponseEntity<ResponseResult<Apprentice>> add(
    ...
    /**
     * get – осуществляет получение всех учеников
     */
    @GetMapping
    ...
    /**
     * get – осуществляет получение ученика по его id
     */
    @GetMapping(path = "/{id}")
    ...
    public ResponseEntity<ResponseResult<Apprentice>> update(@RequestBody Apprentice apprentice) {
        try {
            ...
            Apprentice apprenticeOld = this.apprenticeService.update(apprentice);
            return new ResponseEntity<>(new ResponseResult<>(null, apprenticeOld), HttpStatus.OK);
        } catch (IllegalArgumentException e) {
            return new ResponseEntity<>(new ResponseResult<>(e.getMessage(), null),
                    HttpStatus.BAD_REQUEST);
        }
