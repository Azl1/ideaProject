...
public class HistoryRepository {
    ...
    public HistoryRepository() {
        ...
        Retrofit retrofit = new Retrofit.Builder()
                ...
                .client(client)
                .build();
        this.historyService = retrofit.create(HistoryService.class);
    }

    public HistoryRepository(String token) {
        objectMapper = new ObjectMapper();
        objectMapper.registerModule(new JavaTimeModule());
    ...
    public List<History> getHistory(long idQuiz) throws IOException {
        Response<ResponseResult<List<History>>> execute = this.historyService.getListQuiz(idQuiz).execute();
        return getData(execute);
    }

    public History post(History history) throws IOException {
        retrofit2.Response<ResponseResult<History>> execute = this.historyService.post(history).execute();
        return getData(execute);
    }
}
